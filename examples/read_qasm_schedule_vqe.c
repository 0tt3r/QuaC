...
int main(int argc,char **args){
  ...
  PetscScalar mat_val;
  char filename[256],outfile[256],outfile2[256],file_ev[256];
  /* Initialize QuaC */
  ...
  PetscOptionsGetString(NULL,NULL,"-file_ev",file_ev,sizeof(file_ev),NULL);
  ...
  /* printf("Fidelity: %f\n",fidelity); */
  PetscScalar trace_val,*evs;
  PetscInt num_evs;
  qiskit_vqe_get_expectation(file_ev,rho2,&trace_val,&num_evs,&evs,system);
  printf("ev: %12.20e\n",PetscRealPart(trace_val));
  printf("strings: ");
  for(i=0;i<num_evs;i++){
    printf("%f ",evs[i]);
  }
